        ...
        <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
            ...
            <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
                ...
                <form onSubmit={onSubmitHandler}>
                    ...
                    {state === 'Login' && (<p onClick={() => navigate('/reset-password')} className='mb-4 text-indigo-500 cursor-pointer'>
                        Forgot password?
                    </p>)}
                ...
                <p onClick={() => setState(state === 'Sign Up' ? 'Login' : 'Sign Up')} className='text-gray-400 text-center text-xs mt-4'>
                    {state === 'Sign Up' ? 'Already have an account?' : 'Don\'t have an account?'} {' '}
                    <span className='text-blue-400 cursor-pointer underline'>{state === 'Sign Up' ? 'Login' : 'Sign Up'}</span>
                </p>
            </div>
        </div>
    )
